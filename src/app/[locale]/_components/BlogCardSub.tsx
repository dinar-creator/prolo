...
export default async function BlogCardSub({ thumbnail, heading, slug, date }: BlogCard) {
  const locale = await getLocale();
  const ctas = await getTranslations("ctas");
  return (
    <div className="flex w-full flex-col items-start justify-start gap-4 rounded-xl md:flex-row">
      {/* Image */}
      <div className="aspect-2/1 h-[200px] w-full md:w-auto">
        <Image
          src={thumbnail}
